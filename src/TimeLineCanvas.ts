...
export default class TimeLineCanvas {
    ...
    private selectedClip: {TimelineClip:TimelineClip,clip:ClipsData} | null = null; // 選択されたクリップ
    ...
    private setupEventListeners(): void {
        ...
        this.canvas.addEventListener('contextmenu', (event) => this.onMouseContextMenu(event));
    ...
    private onMouseMove(event: MouseEvent): void {
        if (this.isDragging && this.selectedClip) {
            ...
            const newStart = (x / this.canvas.width) * this.totalTime + this.scrollOffset / this.canvas.width * this.totalTime;
            const clipLength = this.selectedClip.clip.end - this.selectedClip.clip.start;
    ...
    private getClipAtPosition(x: number, y: number): {clips:TimelineClip,clip:ClipsData} | null {
        let returnData = null

        this.clips.map((clips) => {
            return clips.clips.map((clip) => {
                const xStart = (clip.start / this.totalTime) * this.canvas.width - this.scrollOffset;
                const xEnd = (clip.end / this.totalTime) * this.canvas.width - this.scrollOffset;
                const clipYStart = clips.track * this.trackHeight;
                const clipYEnd = clipYStart + this.trackHeight;

                if (x >= xStart && x <= xEnd && y - this.topPadding >= clipYStart && y - this.topPadding <= clipYEnd) {
    ...
    private drawTimeIndicators(scrollOffset: number): void {
        ...
        for (let i = -scrollOffset; i < this.canvas.width + scrollOffset; i += step) {
    ...
    private drawVerticalGrid(scrollOffset: number): void {
        ...
        for (let i = -scrollOffset; i < this.canvas.width + scrollOffset; i += 10) {
            const x = i + scrollOffset;
    ...
    private drawClips(scrollOffset: number): void {
        ...
        this.clips.forEach((clips) => {
            clips.clips.forEach((clip) => {
                ...
                const xStart = (start / this.totalTime) * this.canvas.width - scrollOffset;
                const xEnd = (end / this.totalTime) * this.canvas.width - scrollOffset;
                const y = track * this.trackHeight + this.topPadding;

                this.ctx.fillStyle = '#1E90FF';
                this.ctx.fillRect(xStart, y + 5, xEnd - xStart, this.trackHeight - 10);
            })
        });
    }
