...
export class MmdManager{
    ...
    async loadAvatar(file: File) {
        const mmdMesh = await SceneLoader.ImportMeshAsync(undefined, "", file, this.scene)
    ...
    async loadStage(file: File) {
        const mmdMesh = await SceneLoader.ImportMeshAsync(undefined, "", file, this.scene).then((result) => result.meshes[0]);
