...
export class MeshManager {
    ...
    getMeshByUniqueID(uniqueId: number) {
        const mesh = this.allMeshs.filter(
            (object) => object.mesh.uniqueId === uniqueId
        )[0]
        if (mesh) {
            return mesh
        } else {
    ...
    addMeshFile(file: File) {
        if (file.name.endsWith('.obj') || file.name.endsWith('.fbx')) {
            SceneLoader.ImportMeshAsync('', file.name, file, this.scene).then(
                (r) => {
    ...
    removeMesh(mesh: Mesh) {
        const index = this.allMeshs.indexOf(
            this.getMeshByUniqueID(mesh.uniqueId)!
        )
        if (index > -1) {
