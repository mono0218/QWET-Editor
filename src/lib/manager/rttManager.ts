...
import {FreeCamera, MeshBuilder, RenderTargetTexture, StandardMaterial, Vector3} from "@babylonjs/core";
...
export class RttManager{
    ...
    async addRtt(){
        const camera = new FreeCamera("camera2", new Vector3(0, 0, 0), this.editor.scene);
        const renderTarget:RenderTargetTexture = new RenderTargetTexture("depth", 1024, this.editor.scene, true);
        renderTarget.activeCamera = camera;
        this.editor.scene.customRenderTargets.push(renderTarget);

        const plane = MeshBuilder.CreatePlane("map", {width:30, height:30, size:30}, this.editor.scene);
        const rttMaterial = new StandardMaterial("RTT material", this.editor.scene);
