...
export default function AvatarCard(data: AvatarCardType) {
  return (
    <>
      <Link
        href={`https://hub.vroid.com/characters/${data.character_model.character.id}/models/${data.character_model.id}`}
        target={"_blank"}
      >
        <div className="flex felx-col max-w-[170px]">
          <Image
            isZoomed
            src={data.character_model.portrait_image.w600.url2x}
          ></Image>
        </div>
