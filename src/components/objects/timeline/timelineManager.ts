...
export class TimeLineManager {
    ...
    init(element: HTMLElement) {
        ...
        this.timelineEditor = new Timeline(this.timelineOptions, {
    ...
    play() {
        ...
        setInterval(() => {
            ...
            const data = this.timelineData.filter(
    ...
    addMmdMotion(name: string, max: number, avatarClass: AvatarComponent) {
        ...
        this.timelineEditor?.setModel({ rows: this.timeLineEditorData })
    }

    selectMode() {
        if (this.timelineEditor) {
            this.timelineEditor.setInteractionMode(
